...
interface HandZoneProps {
  ...
  readonly onMoveCard: (cardId: string, to: "battlefield" | "graveyard") => void;
}
...
export function HandZone({ hand, libraryCount, onDrawCard, onMoveCard }: HandZoneProps) {
  ...
  function handleCardClick(card: DeckCard) {
    setContextMenu(contextMenu?.cardId === card.id ? null : { cardId: card.id, cardName: card.name });
  }
  ...
  return (
    <div className="bg-[var(--surface)] rounded-xl p-4 space-y-3">
      ...
      <div className="flex items-center justify-between">
        <h3 className="text-white font-semibold text-sm">Hand: {hand.length} cards</h3>
        ...
        <div className="flex flex-wrap gap-2">
          {hand.map((card) => (
            <div key={card.id} className="relative">
              ...
              >
                <Image
                  src={showCards ? (card.imageUri || CARD_BACK_URL) : CARD_BACK_URL}
              ...
              {showCards && (
                <p className="text-[9px] text-white/60 text-center truncate max-w-14 mt-0.5">{card.name}</p>
              )}
              ...
              {contextMenu?.cardId === card.id && (
                <div className="absolute bottom-full left-0 mb-1 bg-[var(--surface)] border border-white/20 rounded-lg shadow-xl z-10 min-w-[140px] overflow-hidden">
                  ...
                  <button
                    onClick={() => { onMoveCard(card.id, "battlefield"); setContextMenu(null); }}
                    className="w-full text-left px-3 py-2 text-xs text-white hover:bg-white/10 transition-colors"
                  >
                    → Battlefield
                  </button>
                  <button
                    onClick={() => { onMoveCard(card.id, "graveyard"); setContextMenu(null); }}
                    className="w-full text-left px-3 py-2 text-xs text-white hover:bg-white/10 transition-colors"
                  >
                    → Graveyard
                  </button>
