...
export function SearchFilters({
  ...
  const collectionSize = useCollectionStore(
    (s) =>
      ...
      Object.keys(s.collectionCardsFoil).length
  );

  const [presets, setPresets] = useState<FilterPreset[]>(loadPresets);
  const [presetName, setPresetName] = useState("");

  const showPowerToughness = filters.types.some(
  ...
  const toggleColorless = () => {
    ...
    onChange({
      ...filters,
      colorlessFilter: next,
      colors: next ? [] : filters.colors,
    });
  };

  const toggleLands = () =>
    onChange({ ...filters, landFilter: !filters.landFilter });
      ...
      <div className="flex gap-2 items-center">
        ...
        <select
          ...
          onChange={(e) =>
            onChange({
              ...filters,
              interactionType: (e.target.value as InteractionType) || null,
            })
          }
          className={cn(inputCls, "flex-1")}
        >
