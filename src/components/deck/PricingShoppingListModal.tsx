...
export function PricingShoppingListModal({
  ...
  const subtotal = sortedCards.reduce((sum, c) => sum + (c.price ?? 0) * c.quantity, 0);
  ...
  async function handleCopy() {
    const lines = sortedCards.map(
      (c) => `${c.quantity}x ${c.name} — €${((c.price ?? 0) * c.quantity).toFixed(2)}`
    );
  ...
  return (
    ...
    >
      <div
        className="bg-[var(--background)] rounded-xl shadow-2xl max-w-lg w-full max-h-[80vh] overflow-hidden flex flex-col"
        ...
        <div className="flex-1 overflow-y-auto">
          <table className="w-full text-sm">
            <thead className="bg-white/5 sticky top-0">
              <tr>
                <th className="text-left px-4 py-2 text-white/50 font-medium">Card</th>
                <th className="text-center px-3 py-2 text-white/50 font-medium">Qty</th>
                <th className="text-right px-3 py-2 text-white/50 font-medium">Unit</th>
                <th className="text-right px-4 py-2 text-white/50 font-medium">Total</th>
              </tr>
            </thead>
            <tbody>
              {sortedCards.map((card) => (
                ...
                >
                  ...
                  <td className="px-3 py-2 text-center text-white/60">{card.quantity}</td>
        ...
        <div className="border-t border-white/10 p-4 space-y-3">
          {cardsWithoutPrice > 0 && (
            <p className="text-xs text-white/40">
              {cardsWithoutPrice} card{cardsWithoutPrice > 1 ? "s" : ""} — unknown price (excluded)
            </p>
          )}
