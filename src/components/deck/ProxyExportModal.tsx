...
${chunkArray(slots, perPage)
  .map(
    (page) =>
      `<div class="page">${page.map((slot) => renderCardSlot(slot, imageMap, config.includeCardArt)).join("")}<div class="page-footer">Unofficial proxy — Not for sale — MagicAIBuilder</div></div>`
  )
  .join("\n")}
</body>
</html>`;
