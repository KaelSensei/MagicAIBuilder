...
function makeDeckCard(scryfallCard: ScryfallCard): DeckCard {
  ...
  if (scryfallCard.card_faces && scryfallCard.card_faces.length >= 2) {
    baseCard.cardFaces = [
      {
        ...
        imageUri: scryfallCard.card_faces[0].image_uris?.normal ?? getCardImageUri(scryfallCard, "normal"),
        artCropUri: scryfallCard.card_faces[0].image_uris?.art_crop ?? getCardImageUri(scryfallCard, "art_crop"),
      },
      {
        ...
        imageUri: scryfallCard.card_faces[1].image_uris?.normal ?? getCardImageUri(scryfallCard, "normal"),
        artCropUri: scryfallCard.card_faces[1].image_uris?.art_crop ?? getCardImageUri(scryfallCard, "art_crop"),
      },
    ];
  }
  if (isMdfcWithLandBack(scryfallCard)) {
    baseCard.isFlexibleLand = true;
  }
  return baseCard;
}
