...
function isCardDraw(card: ScryfallCard): boolean {
  const text = (card.oracle_text ?? "").toLowerCase();
  return text.includes("draw a card") || text.includes("draw two cards") ||
    text.includes("draw three cards") || text.includes("draw x cards") ||
...
function isTutor(card: ScryfallCard): boolean {
  const text = (card.oracle_text ?? "").toLowerCase();
  return text.includes("search your library") && !text.includes("basic land") && !text.includes("land card");
}

function isWinCondition(card: ScryfallCard): boolean {
  const text = (card.oracle_text ?? "").toLowerCase();
  const keywords = card.keywords?.map((k) => k.toLowerCase()) ?? [];
  return text.includes("win the game") || text.includes("you win the game") ||
    text.includes("infinite") || keywords.includes("infect") ||
...
function isProtection(card: ScryfallCard): boolean {
  const text = (card.oracle_text ?? "").toLowerCase();
  const keywords = card.keywords?.map((k) => k.toLowerCase()) ?? [];
  return (text.includes("hexproof") && card.cmc <= 3) ||
    (text.includes("shroud") && card.cmc <= 3) ||
    keywords.includes("counterspell") || text.includes("counter target") ||
...
export const CATEGORY_LABELS: Record<CardCategory, string> = {
  commander: "Commander", companion: "Companion", creature: "Creatures",
  instant: "Instants", sorcery: "Sorceries", artifact: "Artifacts",
  enchantment: "Enchantments", planeswalker: "Planeswalkers", land: "Lands",
  ramp: "Ramp", draw: "Card Draw", removal: "Removal", boardWipe: "Board Wipes",
  winCondition: "Win Conditions", protection: "Protection", other: "Other",
};

export const CATEGORY_ORDER: readonly CardCategory[] = [
  "commander","creature","ramp","draw","removal","boardWipe","winCondition",
  "protection","instant","sorcery","artifact","enchantment","planeswalker","land","other",
];
