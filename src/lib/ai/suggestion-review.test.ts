...
const suggestion = (name: string, priority: CardSuggestion["priority"]): CardSuggestion => ({
  name,
  priority,
  reason: `${name} reason`,
  category: "ramp",
...
const removal = (name: string): CardRemoval => ({ name, reason: `${name} reason` });

describe("filterSuggestionsByPriority", () => {
  it("keeps only suggestions matching the selected priority", () => {
    const suggestions = [suggestion("A", "high"), suggestion("B", "low"), suggestion("C", "high")];
    const filter: SuggestionPriorityFilter = "high";

    expect(filterSuggestionsByPriority(suggestions, filter).map((item) => item.name)).toEqual(["A", "C"]);
  });

  it("returns all suggestions for the all filter", () => {
    const suggestions = [suggestion("A", "high"), suggestion("B", "low")];

    expect(filterSuggestionsByPriority(suggestions, "all")).toEqual(suggestions);
  });
});
