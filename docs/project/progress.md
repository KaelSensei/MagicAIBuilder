...
## Current metrics (2026-09-17)

| Metric              | Value                                                                                                                                   |
| ------------------- | --------------------------------------------------------------------------------------------------------------------------------------- |
| Unit tests          | 2680 across 212 files                                                                                                                   |
...
- The next playtest priorities are surfacing missing colors and dead opening hands, then associating player notes with proposed deck changes.
...
- ~~**Two hooks shipped in #528 and #529 have no tests.**~~ **Resolved 2026-08-22 (#534).** `useApiKeys` was at **0% of 88 statements** — the largest uncovered unit in the repository, and the one that mints and revokes credentials — and `useMetaShifts` at 25.6%. Coverage **93.69% → 94.89%**, back above the 94.5% that preceded the batch. The property worth naming: **a freshly minted token must survive a failed refetch**, because it exists exactly once, in the response that created it, and the row stores only a hash. `createKey` sets it before reloading the list for that reason, and a test now fails if the order is reversed.
  - **Four older hooks remain at 0%** (`useLocalizedCardText`, `useScrollReveal`, `useTypewriter`) along with the two `instrumentation` files. They were already uncovered before that batch and were left alone.
- **The e2e gate spent months measuring something other than what it claimed** (2026-08-22, #535). `playwright.config.ts` keyed `forbidOnly`, `retries` and `workers` on `process.env.CI`, which **nothing sets** — not the e2e container, and no workflow runs Playwright at all. So `forbidOnly` was never enforced: a stray `test.only` would have narrowed the blocking gate to one test and still reported green. This is the **fourth** silent-green shape catalogued in this repository, after `PLAYWRIGHT_SPEC`, the skipped Sonar scan, and the agent-verdict gate that exited 0 on 27 failures.
  - **The rule that keeps falling out of these**: a gate whose behaviour depends on an environment variable is a gate that can be disabled by not setting it, silently. State the value outright unless something genuinely varies.
  - `trace: "on-first-retry"` could never fire either, which is why every gate failure to date produced only an aria snapshot — no network log, no DOM timeline. Now `retain-on-failure`.
...
- ~~**`--accent` is the last colour-contrast failure.**~~ **Resolved 2026-08-21 (#519).** Split into `--accent` (backgrounds) and `--accent-text`, 91 call sites swept; accessibility is 100 on every audited page. Two further defects came out of it: `--accent-hover` at **2.98:1** under white text in the dark theme — worse than the resting state, and invisible to any audit, since Lighthouse never measures a hover — and light `--text-secondary` at 4.28:1 on `--surface-hover`. The ratchets rose to 0.95.
- **Contrast rules that only hold at one extreme keep passing review.** Three instances now: `#7c5cfc` cleared 4.5:1 on `--black` and failed on `--surface` (#516); light `--text-secondary` cleared it on `#ffffff` and failed on `#f1f3f5` (#519, my own first solve); `--accent` cleared neither. **The binding surface is the one closest in luminance to the text, not the obvious extreme** — darkest for dark themes, lightest for light ones. Both `contrast.test.ts` files assert against every surface for this reason.
- **An audit measures the page as rendered and nothing else.** `--accent-hover` shipped at 2.98:1 and Lighthouse scored the page 96. Hover, focus and active states have no coverage at all; they are only reachable by computing the ratio from the tokens, which is what `src/styles/contrast.test.ts` now does.
- ~~**`sitemap.ts` lists only `/` and `/share/<token>`.**~~ **Resolved 2026-08-21 (#521)** — and the incompleteness was the lesser half. It was **publishing capability URLs**: every `shareEnabled` deck's token went to search engines, though the sharing dialog promises "anyone with this link can view your deck" and the route serving it checks `shareEnabled`, never `isPublic`. A privately-shared deck was therefore indexable. `robots.ts` had a matching `allow: "/share/"`. Both removed; public decks, profiles and commander pages added; the bare `catch {}` now reports through `logger.error`.
- ~~**Two environment variables name the same thing.**~~ **Correction (2026-08-21, #524): they do not, and merging them would have introduced a bug.** `resolveAppBaseUrl()` resolves _the instance that is running_ — a server component calling this app's own API — so it falls back to `VERCEL_URL`, the preview host on a preview. `siteUrl()` is the site's _public identity_, used for canonicals, hreflang and the sitemap; resolving it to a preview host would publish canonical URLs pointing at deployments that disappear, which is exactly the damage #517 and #521 repaired. **On a preview the two are supposed to disagree.** The claim survived several restatements in this file, including in the entry that proposed reconciling them, because the names look interchangeable — which is the whole hazard. Each function now points at the other and says why they stay apart. Found by going to merge them, not by review.
...
- ~~**A client disconnect is logged as a 500.**~~ **Resolved 2026-08-21 (#522).** Nineteen routes shared the shape, not one — `readJsonBody` returns a result the caller cannot ignore. Three of them (`cache/cards`, `cache/search`, `decks/[id]/snapshots`) stopped compiling when the body became `unknown` instead of `any`: they destructured it directly. They _do_ validate the fields immediately afterwards, so it was a typing hole rather than a validation hole — but `any` is what kept it open. `ai/build`, `ai/suggest` and `import/url` already handled it and were left alone.
- **A visibility rule can be wrong in a state the schema permits and nothing will say so.** `findVisibleDeck` compared `deck.userId !== viewerId` with both sides nullable, so an ownerless private deck read as owned by an anonymous viewer. `Deck.userId` is optional, but the only creation path sets it and no such row exists — **latent, never live**. It was byte-identical in four routes and tested in none; fixed and given seven tests in #522. Found by a test written as a hypothesis, not by an audit.
- **Duplication that predates a change becomes the change's problem.** SonarCloud failed #522 at 3.8% new-code duplication. The added lines were three per route, under the detector's block size; the duplications API pointed instead at 21-to-34-line blocks of pre-existing community boilerplate that merely got pulled into the new-code measure. **Ask the API which block, rather than refactoring the lines you just wrote.**
...
- ~~**Intermittent e2e failure**~~ **Diagnosed and mitigated 2026-08-20 (#495).** The mechanism was in the harness, not the app: `openBuilder` returned as soon as `waitForURL` saw the address change, before the route was usable — deck store still syncing, React still hydrating. A click in that window is **swallowed**: the listener has attached and calls `preventDefault`, but the router cannot act yet, so the native navigation is cancelled and nothing replaces it. That is load-sensitive by construction, which explains every observation, including hypothesis 3 being falsified — compile timing was a _trigger_, never the mechanism. The helper now waits for the loaded branch and `networkidle`.
  ...
  - **2026-08-22 (#535): most of the load was self-inflicted, and the advice never said so.** Every recurrence was attributed to a busy host, and the standing guidance was to avoid other Docker builds during the gate. Measured across the same 58 tests, each took **8–13 s under parallel load against ~2.5 s serially** — a fourfold inflation the suite was inflicting on itself with no external help. The gate now runs on one worker. That is fixing the flake rather than padding it, which is why retries stay at 0.
  - **Cost: ~3.4 min serial against 1.7 min parallel.** Both figures are stopwatch readings on full runs. Two earlier estimates derived from per-test durations were wrong in opposite directions — 8–10 min (reading parallel timings at face value) and 2m24 (summing serial ones). **Measure the whole run.**
