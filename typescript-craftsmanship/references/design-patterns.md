...
function useDeckData(deckId: string) { /* fetch + cache deck */ }
function useDeckValidation(deck: Deck) { /* validate + return errors */ }
function useCardSearch(query: string) { /* search Scryfall */ }
function useDeckViewPrefs() { /* UI preferences */ }
```
...
interface DeckReader { decks: readonly Deck[]; }
...
class DeckBuilder {
  ...
  withName(name: string): this { this.deck.name = name; return this; }
  withCommander(card: DeckCard): this { this.deck.commander = card; return this; }
  withPartner(card: DeckCard): this { this.deck.partner = card; return this; }
...
function createValidator(format: DeckFormat): DeckValidator {
  switch (format) {
    case "commander": return new CommanderValidator();
    case "standard": return new StandardValidator();
    case "modern": return new ModernValidator();
  }
...
```typescript
type FilterNode =
  | { type: "condition"; field: string; operator: "eq" | "gt" | "lt"; value: string }
...
const CARD_SORTS: Record<string, SortStrategy<DeckCard>> = {
  ...
  color: (a, b) => a.colorIdentity.join("").localeCompare(b.colorIdentity.join("")),
};
...
class AddCardCommand implements Command {
  ...
  execute() { this.store.addCard(this.deckId, this.card); }
  undo() { this.store.removeCard(this.deckId, this.card.id); }
  describe() { return `Add ${this.card.name}`; }
}
...
function useDropZone<T>(options: {
  ...
  const handlers = useMemo(() => ({
    onDragOver: (e: DragEvent) => { e.preventDefault(); setIsOver(true); },
