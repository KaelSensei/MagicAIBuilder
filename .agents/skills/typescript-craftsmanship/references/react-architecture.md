...
function useDeckActions(deckId: string) {
  ...
  return useMemo(
    () => ({
      ...
      updateQty: (cardId: string, qty: number) =>
        updateQuantity(deckId, cardId, qty),
    }),
...
// deck-slice.ts
const createDeckSlice: StateCreator<AppStore, [], [], DeckSlice> = (
  set,
  get
) => ({
  decks: [],
...
const activeDeck = useAppStore(
  (s) => s.decks.find((d) => d.id === s.activeDeckId) ?? null
);
...
describe("useDeckStore", () => {
  ...
  it("prevents adding cards that violate color identity", () => {
    ...
    const deckId = useDeckStore
      .getState()
      .createDeck("Mono W", monoWhiteCommander);
